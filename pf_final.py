...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        alpha = random.gauss(motion[0],res.steering_noise)
        d = random.gauss(motion[1], res.distance_noise)
        
        beta = tan(alpha)*d/res.length
        
        if beta > 0:
            radius = res.length /tan(alpha)
            cx = res.x - radius*sin(res.orientation)
            cy = res.y + radius*cos(res.orientation)
            
            res.x = cx + radius*sin(res.orientation + beta)
            res.y = cy - radius*cos(res.orientation + beta)
        else:
            res.x += d*cos(res.orientation)
            res.y += d*sin(res.orientation)
        res.orientation += beta
    ...
    def sense(self,flag): #do not change the name of this function
        ...
        for landmark in landmarks:
            del_x = landmark[1]-self.x
            del_y = landmark[0]- self.y
            res = (atan2(del_y,del_x)-self.orientation)%(2*pi)
            if flag == 1:
